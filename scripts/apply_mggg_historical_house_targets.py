#!/usr/bin/env python3
"""Audit or apply MGGG-projected results for selected modern House districts."""
...
def main() -> int:
    ...
    parser.add_argument("--contests", default="")
    ...
    requested = requested or set(DEFAULT_CONTESTS)
    ...
    for contest in sorted(requested):
        ...
        if args.write and file_changed:
            ...
            if isinstance(meta, dict):
                meta["targeted_mggg_projection_districts"] = benchmark.get("target_districts", [])
                meta["targeted_mggg_projection_source"] = benchmark.get("source")
                meta["targeted_mggg_projection_method"] = benchmark.get("method")
    ...
    print(json.dumps({key: report[key] for key in ("mode", "plan", "contests", "changed_rows", "missing")}, indent=2))
