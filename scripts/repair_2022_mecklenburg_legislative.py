#!/usr/bin/env python3
"""Patch 2022-line Mecklenburg legislative 2008 Governor districts.

Uses the corrected 2024-line district vote shares as the source, while keeping
2022-line JSON total_votes fixed. This fixes bad flat Mecklenburg allocations
without changing the 2022-line turnout scale.
"""
...
PATCHES = [
    {
        "target": "data/district_contests/state_senate_governor_2008.json",
        "source": "data/district_contests_2024_lines/state_senate_governor_2008.json",
        "districts": ["38", "39", "40", "41", "42"],
    },
    {
        "target": "data/district_contests/state_house_governor_2008.json",
        "source": "data/district_contests_2024_lines/state_house_governor_2008.json",
        "districts": ["98", "99", "100", "101", "102", "103", "104", "105", "106", "107", "108", "109", "110", "111", "112"],
    },
]
...
def main() -> None:
    ...
    for patch in PATCHES:
        ...
        source_path = Path(patch["source"])
        target_text = target_path.read_text(encoding="utf-8")
        target = json.loads(target_text)
        source = json.loads(source_path.read_text(encoding="utf-8"))
        target_results = target["general"]["results"]
        source_results = source["general"]["results"]
        for district in patch["districts"]:
            update_target_row(target_results[district], source_results[district])
        pretty = "\n" in target_text.strip() and len(target_text.strip().splitlines()) > 1
        out = json.dumps(target, indent=2, ensure_ascii=False) + "\n" if pretty else json.dumps(target, separators=(",", ":"), ensure_ascii=False) + "\n"
        target_path.write_text(out, encoding="utf-8")
        summary.append({"target": patch["target"], "districts": patch["districts"]})
    print(json.dumps({"patched": summary}, indent=2))
