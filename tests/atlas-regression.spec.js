...
test('source confidence panel stays hidden when a contest is selected', async ({ page }) => {
  ...
  await page.waitForFunction(() => document.getElementById('vote-counter')?.style.display === 'block', null, { timeout: APP_READY_TIMEOUT });
  ...
  for (const [senatePayload, senateClusters] of [[payloads[2], senateClusters2022], [payloads[5], senateClusters2024]]) {
    ...
    for (const [district, countyNames] of Object.entries(senateClusters)) {
      const expected = countyNames.reduce((sum, county) => ({
        dem_votes: sum.dem_votes + Number(counties[county].dem_votes || 0),
        rep_votes: sum.rep_votes + Number(counties[county].rep_votes || 0),
        other_votes: sum.other_votes + Number(counties[county].other_votes || 0)
      }), { dem_votes: 0, rep_votes: 0, other_votes: 0 });
      expect(senatePayload.general.results[district]).toMatchObject(expected);
    }
  }
});
...
test('comparison cards name both contests instead of using opaque A/B keys', async ({ request }) => {
  ...
  const buildId = source.match(/const APP_BUILD_ID = '([^']+)'/)?.[1] || '';
  expect(source).toContain(`atlas-comparison.js?v=${buildId}`);
