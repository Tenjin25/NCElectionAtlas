...
test('normalizes supported view, mode, and line tokens', () => {
  ...
  assert.equal(AtlasURLState.normalizeModeToken(' COMPARISON '), 'comparison');
...
test('parses a complete shareable atlas URL state', () => {
  assert.deepEqual(
    AtlasURLState.parse(
      '?view=counties&contest=us_senate_model_2026&compare=president_2024&mode=comparison&focus=county%3AWAKE' +
