...
- **Unopposed Filtering (Counties):** Unopposed Council of State contests and uncontested / same-party-only judicial contests are hidden from the Counties picker
	...
	- **Optional DRA-Inspired 15-Tier Palette:** A `DRA Colors` option under `More` preserves the original atlas palette as the default and retains its dark Stronghold/Dominant/Annihilation colors in the alternate palette. DRA's published Safe and competitive colors are fitted to the existing margin-of-victory tiers; its 40%/45%/50%/55%/60% Democratic vote-share anchors correspond to 20/10/0/10/20-point margins.
