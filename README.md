...
- **Unopposed Filtering (Counties):** Unopposed Council of State contests and uncontested / same-party-only judicial contests are hidden from the Counties picker
	...
	- **Optional DRA-Inspired 15-Tier Palette:** A `DRA Colors` option under `More` preserves the original atlas palette unchanged as the default while offering a coherent alternate based on DRA's published packed, Safe, and competitive colors. DRA's 40%/45%/50%/55%/60% Democratic vote-share anchors correspond to 20/10/0/10/20-point margins; its packed colors are extended for the atlas's extra top tiers, and partisan fills use a softer DRA-style basemap blend.
