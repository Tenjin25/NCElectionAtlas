...
- County-facing election results use canonical `county_totals` (and compact files in `data/county_contests/`) when available. They are not recomputed from crosswalked precinct rows, which can retain small allocation residuals even when the crosswalk is otherwise high quality.
- Rebuild compact county slices from OpenElections CSVs (SBE TSV conversions) with `python scripts/build_county_contests_from_oe.py --write`. The builder applies `data/mappings/judicial_candidate_party_overrides.csv` so nonpartisan / mislabeled judicial candidates (for example 2018 Anglin → OTHER) keep correct DEM/REP county margins.
- **July 16, 2026 bridge clamp:** Dec 2025 OneMap VAP bridges no longer move votes across county lines. Run `python scripts/clamp_precinct_bridge_to_source_county.py --write`, then rebuild statewide contests with `node scripts/rebuild_statewide_contests_from_sbe_bridge.js` (defaults cover **2000–2024**, including VTD00/VTD10 early-era bridges and SBE 2012+). After that rebuild, precinct-row county sums match OE county totals again (Northampton/Halifax/Catawba/etc.), so county-layer accuracy no longer depends on the dual-truth workaround.
- The newer December 2025 OneMap/SBE crosswalk outputs remain unchanged and continue to drive precinct-level detail. This is an intentional two-layer contract: authoritative totals for county polygons and statewide county summaries, crosswalked rows for the precinct layer underneath.
