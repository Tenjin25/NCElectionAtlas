...
- **Unopposed Filtering (Counties):** Unopposed Council of State contests and uncontested / same-party-only judicial contests are hidden from the Counties picker
	...
	- **Optional Smooth DRA 15-Tier Palette:** A `DRA Colors` option under `More` preserves the original atlas palette unchanged as the default while offering a balanced DRA-inspired alternate across Margins, Winners, Shift, and Flips in county, precinct, and district views. The palette is adapted directly to the atlas's 40/30/20/10/5.5/1/0.5-point margin thresholds, with darker stronghold colors and smoother competitive transitions. Demographics and Population Change retain their own semantic palettes.
